  ...
  return (
    <form action={saveTheme} className="grid gap-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="text-sm font-bold text-[#647067]">Saved theme powers the storefront home page</p>
          <h1 className="text-3xl font-black">Theme editor</h1>
        </div>
        <button className="rounded-lg bg-[#173326] px-4 py-2 text-sm font-black text-white">Save theme</button>
      </div>
        ...
        <div className="grid gap-3">
          {sections.map((section) => (
            <div key={section.id} className="rounded-lg border border-[#dfe7df] bg-white p-4">
              ...
              <label className="mt-3 flex items-center gap-2 text-sm font-bold"><input name={`${section.id}.visible`} type="checkbox" defaultChecked={section.visible} /> Visible</label>
            </div>
          ))}
        </div>
        <div className="rounded-lg border border-[#dfe7df] bg-[#fbfaf6] p-5">
          <p className="mb-3 text-sm font-black text-[#647067]">Live preview</p>
