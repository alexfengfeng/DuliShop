...
export default async function AppsPage() {
  ...
  return (
    <div className="grid gap-5">
      <div>
        <p className="text-sm font-bold text-[#647067]">Install state is persisted in Postgres</p>
      ...
      <section className="grid gap-3 md:grid-cols-3">
        {apps.map((app) => (
          <article key={app.id} className="rounded-lg border border-[#dfe7df] bg-white p-4">
            ...
            <p className="mt-3 text-sm text-[#647067]">Scopes: {app.scopes.join(", ")}</p>
